...
pub fn main() {
    ...
    match image {
        ...
        Ok(image) => {
            resources = Rc::new(Resources { earth: image });
        }
    }
    //default materials
    ...
    */
    ...
    let _material_metal = Rc::new(MetalMaterial::new(col_from_frac(0.8, 0.8, 0.8), 0.05));
    let _material_left = Rc::new(DiffuseMaterial::new(col_from_frac(1.0, 0.0, 0.0)));
    let _material_right = Rc::new(DiffuseMaterial::new(col_from_frac(0.0, 0.0, 1.0)));
    let _material_back_front = Rc::new(DiffuseMaterial::new(col_from_frac(1.0, 1.0, 1.0)));
    let _material_top = Rc::new(EmmissiveMaterial::new(col_from_frac(1.0, 1.0, 1.0)));
        ...
        for pix_x in 0..data.canvas_width {
            ...
            for _i in 0..data.samples {
                let mut vec = claculate_vec_dir_from_cam(
                    &data,
                    (
                        pix_x as f64 + rng.gen_range(0.0..1.0) * 0.0,
                        pix_y as f64 + rng.gen_range(0.0..1.0) * 0.0,
                    ),
                );
                vec.normalize();
